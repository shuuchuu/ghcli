...
def create_gui() -> None:
    ...
    def server(input: Inputs, output: Outputs, session: Session) -> None:  # ruff: ignore[builtin-argument-shadowing]
        ...
        def plot() -> ui.Tag | ui.HTML:
            ...
            df_count = df.groupby("created_at").size().reset_index(name="count")  # ty: ignore[no-matching-overload]
